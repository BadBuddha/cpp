...
void drawRuler(int lengthOfRuler, int numberOfInches, int majorTickLength){
...
void drawRuler(int nInches, int majorLength){   // draw the entire ruler
    drawOneTick(majorLength, 0);        // draw tick 0 and it's label
    for(int i = 1; i <= nInches;i++){
        drawTicks(majorLength-1);       // draw ticks for this inch
        drawOneTick(majorLength, i);    // draw tick i and it's label
    }
}
